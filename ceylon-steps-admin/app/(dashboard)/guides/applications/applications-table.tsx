...
export function ApplicationsTable() {
  const [items, setItems] = useState<GuideApplication[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listGuideApplications({
  ...
  // Refetch with current filters when the sidebar poll detects a guide count
  // change (new submission or one moved off PENDING).
  useEffect(() => {
    function onChanged(e: Event) {
      const detail = (e as CustomEvent<PendingCountsChangedDetail>).detail;
      if (detail?.changed.guides) void refresh();
    }
  ...
  function onApplicationChanged(updated: GuideApplication) {
    setItems((prev) =>
