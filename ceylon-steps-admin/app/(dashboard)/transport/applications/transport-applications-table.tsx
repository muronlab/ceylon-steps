...
import { TransportApplicationDetailSheet } from "./transport-application-detail-sheet"
...
export function TransportApplicationsTable() {
  const [items, setItems] = useState<TransportApplication[]>([])
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await listTransportApplications({
  ...
  // Refetch with current filters when the sidebar poll detects a transport
  // count change (new submission or one moved off PENDING).
  useEffect(() => {
    function onChanged(e: Event) {
      const detail = (e as CustomEvent<PendingCountsChangedDetail>).detail
      if (detail?.changed.transport) void refresh()
    }
  ...
  function onApplicationChanged(updated: TransportApplication) {
    setItems((prev) =>
  ...
  return (
    <div className="flex flex-col gap-3">
            ...
            {loading && items.length === 0 ? (
              ...
              items.map((a) => (
                ...
                >
                  ...
                  <TableCell>
                    {a.hasBusiness ? (
                      <span className="text-xs">{a.businessName ?? "Yes"}</span>
                    ) : (
      ...
      <TransportApplicationDetailSheet
