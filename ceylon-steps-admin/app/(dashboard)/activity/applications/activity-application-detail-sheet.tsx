...
import {
  ...
  updateActivityApplicationStatus,
  type ApplicationStatus,
  type ActivityApplication,
} from "@/lib/activity-applications-api"

export function ActivityApplicationDetailSheet({
  applicationId,
  open,
  onOpenChange,
  onChanged,
  ...
  onChanged: (a: ActivityApplication) => void
}) {
  const [app, setApp] = useState<ActivityApplication | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await getActivityApplication(applicationId)
      setApp(data)
    } catch (err) {
  ...
  async function approve() {
    ...
    setError(null)
    try {
      const updated = await updateActivityApplicationStatus(app.id, {
        status: "APPROVED",
      })
      setApp({ ...app, ...updated })
      onChanged(updated)
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="w-full overflow-y-auto p-0 sm:max-w-2xl min-w-[60vw]">
          <SheetHeader className="border-b p-5">
            <SheetTitle className="text-lg">Activity application</SheetTitle>
              ...
              <div className="flex flex-col gap-5">
                ...
                <section className="grid gap-2 border-t pt-4">
                  ...
                  <KV
                    ...
                    value={app.natureOfBusiness}
                  />
                  <KV label="Address" value={app.address} />
                  {app.description && (
