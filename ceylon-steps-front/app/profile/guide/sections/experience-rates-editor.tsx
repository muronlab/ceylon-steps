...
export function ExperienceRatesEditor({
  profile,
  onSave,
}: {
  ...
  async function save() {
    ...
    if (yearsValue !== null && (!Number.isInteger(yearsValue) || yearsValue < 0 || yearsValue > 80)) {
  ...
  const hasYears = profile.yearsOfExperience !== null && profile.yearsOfExperience !== undefined
  ...
  return (
    <div className="mt-5 rounded-4xl bg-white p-5 ring-1 ring-zinc-200/70">
      <div className="flex items-center justify-between">
        <div className="text-sm font-semibold text-zinc-950">Experience &amp; rates</div>
        {!editing ? (
          <Button
            type="button"
            variant="ghost"
      ...
      {!editing ? (
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          <Stat
            ...
            label="Experience"
            value={hasYears ? `${profile.yearsOfExperience} year${profile.yearsOfExperience === 1 ? "" : "s"}` : "—"}
          />
          <Stat
          ...
          />
          {!hasYears && !hasAnyRate && (
            <div className="sm:col-span-3 rounded-2xl bg-zinc-50 px-4 py-3 text-xs text-zinc-500 ring-1 ring-zinc-200/70">
              Optional — add your experience and pricing so travelers can decide quickly.
            </div>
          )}
        </div>
      ) : (
