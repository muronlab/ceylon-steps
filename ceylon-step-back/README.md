...
| `CSRF_ENABLED` | `false` by default; set `true` to enforce CSRF tokens |
| `CORS_ALLOW_ALL_ORIGINS` | `true` by default to allow any origin |
| `CORS_ALLOWED_ORIGINS` | Comma-separated frontend origins when you want an explicit allow-list |
